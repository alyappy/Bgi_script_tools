...
def get_section_boundary(data):
    pos = -1
    # Attempts to locate the beginning of the text section by finding a return or exit command sequence.
    while True:
...
def get_text_section(text_bytes):
    ...
    texts = [x.decode(bgi_setup.senc, errors='ignore') for x in strings]
    text_section = {addr: text for addr, text in zip(addrs, texts)}
...
def get_code_section(code_bytes, text_section, config):
    ...
    while pos < code_size:
        ...
        if text_addr in text_section:
            ...
            marker, comment, id = 'Z', 'OTHER', None

            if type == config['STR_TYPE']:
                if check(code_bytes, pos, config['TEXT_FCN'], config['NAME_POS']):
                    marker, comment = 'N', 'NAME'
                    id = names.setdefault(text, ids[marker])
                    ids[marker] += 1
                
                elif check(code_bytes, pos, config['TEXT_FCN'], config['TEXT_POS']):
                    marker, comment = 'T', 'TEXT'
                    name_dword = get_dword(code_bytes, pos + config['TEXT_POS'] - config['NAME_POS'])
                    
                    if name_dword != 0:
                        name_addr = name_dword - code_size
                        name = text_section.get(name_addr, 'TEXT')
                        comment = f'TEXT 【{name}】'
                    id = ids[marker]
                    ids[marker] += 1

                elif check(code_bytes, pos, config['RUBY_FCN'], config['RUBYK_POS']):
                    marker, comment = 'T', 'TEXT RUBY KANJI'
                    id = ids[marker]
                    ids[marker] += 1

                elif check(code_bytes, pos, config['RUBY_FCN'], config['RUBYF_POS']):
                    marker, comment = 'T', 'TEXT RUBY FURIGANA'
                    id = ids[marker]
                    ids[marker] += 1

                elif check(code_bytes, pos, config['BKLG_FCN'], config['BKLG_POS']):
                    marker, comment = 'T', 'TEXT BACKLOG'
                    id = ids[marker]
                    ids[marker] += 1
                else:
                    id = others.setdefault(text, ids[marker])
                    ids[marker] += 1
                
                code_section[pos] = (text, id, marker, comment)
